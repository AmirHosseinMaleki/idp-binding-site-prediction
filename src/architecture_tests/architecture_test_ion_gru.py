...
print("="*70)
print("Ion Binding - Bi-GRU Architecture (20 Epochs)")
print("="*70)
...
print(f"\nTraining Bi-GRU for {EPOCHS} epochs...")
best_val_auc = 0
best_epoch = 0
...
for epoch in range(EPOCHS):
    ...
    if val_metrics['AUC'] > best_val_auc:
        ...
        best_epoch = epoch + 1
        torch.save(model.state_dict(), 'ion_gru_model.pt')
...
model.load_state_dict(torch.load('ion_gru_model.pt'))
test_metrics = evaluate_complete(model, test_loader)

print(f"\n{'='*70}")
print(f"Bi-GRU Results:")
