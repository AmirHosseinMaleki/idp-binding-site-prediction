...
struct_loader = DataLoader(struct_test, batch_size=BATCH_SIZE, num_workers=2)
...
print("\n" + "="*60)
print("FINE-TUNED: Pre-trained on Structured + Fine-tuned on DisProt")
print("="*60)
model_ft = BindingNet(input_size).to(DEVICE)
model_ft.load_state_dict(torch.load('finetuned_model.pt', map_location=DEVICE))

print("\nTesting on Structured test data:")
metrics = evaluate_model(model_ft, struct_loader, threshold=0.5)
for k, v in metrics.items():
    print(f"  {k}: {v:.4f}")

print("\nTesting on DisProt test data:")
metrics = evaluate_model(model_ft, disprot_loader, threshold=0.5)
for k, v in metrics.items():
    print(f"  {k}: {v:.4f}")
