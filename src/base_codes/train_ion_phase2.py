...
print("="*60)
print("Ion Phase 2: Training on DisProt Only (IDPs)")
print("="*60)

cfg = load_config()

# train_data = EmbeddingDataset('disprot_ion_train_embeddings.npz')
# val_data = EmbeddingDataset('disprot_ion_val_embeddings.npz')

train_data = EmbeddingDataset(get_embedding_path(cfg, "disprot_ion_train"))
...
for epoch in range(EPOCHS):
    ...
    if val_loss < best_loss:
        best_loss = val_loss
        # torch.save(model.state_dict(), 'ion_phase2_model.pt')
        torch.save(model.state_dict(), get_model_path(cfg, "ion_phase2"))
